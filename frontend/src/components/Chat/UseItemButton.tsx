    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute bottom-full right-0 mb-2 w-48 bg-panel-900 pixel-border-ornate z-50">
          <div className="font-pixel text-xs text-stone-500 px-3 py-2 border-b border-zinc-800 tracking-widest">
            CONSUMABLES
          </div>
          {usableItems.map((item) => (
            <button
              ...
              className="w-full text-left px-3 py-2 font-pixel text-xs text-stone-300 hover:bg-panel-800 transition-all flex items-center justify-between"
            >
              <span>{item.name}</span>
