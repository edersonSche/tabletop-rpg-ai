...
export class OpencodeProvider implements AIProvider {
  ...
  private buildIncrementalPrompt(context: AIContext): string {
    ...
    if (context.currentAction?.action === 'initiate_trade') {
      ...
      lines.push('- type: specialty (e.g., "blacksmith", "alchemist", "general_goods", "apothecary", "curio_merchant", "armorer", "enchanter", etc.)');
      lines.push('- greeting: a line of dialogue in the campaign language');
      lines.push('- coins: how much coin they have to buy from players (reasonable amount based on location)');
