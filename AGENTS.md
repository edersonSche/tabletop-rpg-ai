...
- `game/` — `GameGateway` (action/roll/start/typing/get_state/room:join/allocate_attributes/equip/unequip/use_antidote/initiate_trade/buy_item/sell_item/end_trade), `GameService` (turn orchestration, AI target validation, `buildSceneContext()`, `maybeSummarize()` history summarization, `initiateTrade()` AI-driven merchant generation, `useAntidote()` condition cure), `GameState` (in-memory Map with HP/XP/level engine, XP thresholds, ASI levels, `activeConditions`, `Effect` engine for stat modifiers/hp changes, inventory/coins/equipment system, merchant/trade state), `TurnManager` (lock-per-room, stores `turnSkill`/`turnDc`, blocks actions during active trade)
