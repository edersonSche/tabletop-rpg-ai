...
- `game/` — `GameState` (`game.state.ts`) — Data layer: in-memory `rooms` Map, `playerByUserId` Map, `createRoom`/`getRoom`/`removeRoom`/`restoreCampaign`, `addHistory`/`setTurn`, shared `recomputePlayer()` for AC recalculation, all type definitions (`Player`, `Effect`, `Condition`, `ActiveCondition`, `InventoryItem`, `Merchant`, `MerchantItem`, `GameStateData`, `TickResult`, etc.)
  ...
  - `TurnManager` (`turn.manager.ts`) — **Owner-aware per-room lock** (stores an owner token per room, not a boolean): `acquire(roomId, ownerId)` returns `false` if the lock is already held by any owner (double-acquire, including reentrancy, is rejected), `release(roomId, ownerId)` only clears when the owner matches (non-owner release is a no-op), `isLocked()` for read-only guards. Stores `turnSkill`/`turnDc`, blocks actions during active trade. `canPlayerAct()` gates `handleAction`/`handleRoll`; `canInitiateTrade()` additionally rejects during `call_player`/`call_roll` (trade only in `group_action`). `processTurn`/`initiateTrade` acquire with `playerId`, `startCampaign` with `'start'`; `handleEndTrade` acquires with `playerId` before calling `TradeService.markDone`. **`handleDisconnect` does not touch the turn lock** — its trade cleanup (`removeFromTrade`) is fully synchronous and must never block or steal an in-flight AI lock. `handleEndTrade`/`handleBuyItem`/`handleSellItem` early-return when `isLocked()` (AI in flight) to avoid releasing an in-flight turn lock.
